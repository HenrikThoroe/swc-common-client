...
export default function rateMobility(state: State, phase: GamePhase, mobility: ConcreteAspect<Mobility>): ConcreteAspect<number> {
    const rateDraggable = (pieces: PieceCollection, queenFactor: number) => {
        ...
        const value = summedDraggable / 11
    ...
    return {
        own: (rateUndeployed(mobility.own.undeployed) + rateDraggable(mobility.own.draggable, queenFactor(phase, true))) / 3,
        opponent: (rateUndeployed(mobility.opponent.undeployed) + rateDraggable(mobility.opponent.draggable, queenFactor(phase, false))) / 3
    }
}